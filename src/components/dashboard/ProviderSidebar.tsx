...
import { StakingActionEnum, useStaking } from "@/lib/context/StakingContext";
...
export const ProviderSidebar: React.FC<IProviderSidebarProps> = ({
  ...
  const { state, toggleSelectedAddress, removeAddress, dispatch } =
    useStaking();
  ...
  const handleSelectAll = React.useCallback(() => {
    dispatch({
      ...
      payload: { addresses: addedAddresses },
    });
      ...
      <div className="md:hidden p-4 border-b border-border/50">
        ...
        <div className="flex flex-col gap-2 mb-3 max-h-48 overflow-y-auto pr-2">
          {addedAddresses.map((address) => (
            <div
              key={`sidebar-wallet-${address}`}
              className="flex items-center gap-2 border rounded-md px-2.5 py-1.5 bg-muted/50 text-xs"
            >
              <Checkbox
        ...
        <div className="flex items-center justify-end gap-2">
          <Button
            ...
            disabled={selectedAddresses.length === addedAddresses.length}
          >
            All
          </Button>
          <Button
          ...
          {!providers || providers.length === 0 ? (
            ...
            providers.map((provider) => (
              <Button
                ...
                variant={
                  selectedProviderAddress === provider.provider
                    ? "secondary"
                    : "ghost"
                }
